...
### One option which specifies the run-mode i.e. from clarity extract or from workbooks:
...
* `--clarity_extract`: (path to clarity extract CSV file): If specified, the script will parse the clarity extract file to get sample and workbook information and either use the paths directly (if --use_paths is specified) or output a CSV for review.
  This will contain:
